        ...
        while True: 
            ...
            with tqdm(dataloader['train_mix']) as td:
                for batch_data in td:
                    ...
                    # loss_T_mix = 0.0
                    # text_utt, text_utt_chaotic, text_utt_mix, y, y2, ymix, lam = mixup_data(text_utt, pret)
                    # x_t1 = model.Model.post_text_dropout(text_utt_mix)
                    # x_t2 = F.relu(model.Model.post_text_layer_1(x_t1), inplace=True)
                    # x_t3 = F.relu(model.Model.post_text_layer_2(x_t2), inplace=True)
                    # output_text = model.Model.post_text_layer_3(x_t3)
                    # loss_T_mix += self.args.T * self.criterion(output_text, ymix)
